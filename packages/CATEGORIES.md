...
Before building anything, check here to understand where it fits, what state
it's in, and which team owns it.

The categories also exist so the project can scale to an organisation:
each category is a self-contained surface a small team can own end-to-end
(component + adapters + theme support + framework wrappers + docs).
...
| `@wick/react` | 🧩 | React wrappers for all ✅ components |
